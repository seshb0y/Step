...
import { calculateTotal, changeAmount, deleteFromBasket, updateBasket, useDiscount } from '../redux/action'
...
const BasketList = () => {
    ...
    const handleChangeAmount = (id: number, isIncrease: boolean) => {
        dispatch(changeAmount({id, isIncrease: isIncrease}))
    }
  ...
  return (
    <div>
        <ul>
            {
                basket.filter((product) => product.amount >= 1).map((product) => (
                    <li key={product.id}>
                        {product.name}
                        ...
                        <button onClick={()=> handleChangeAmount(product.id, true)}>+</button>
                        <button onClick={()=> handleChangeAmount(product.id, false)}>-</button>
                        <button onClick={() => handleDeleteFromBasket(product.id)}>Delete</button>
                    </li>
                ))
            }
        </ul>
        <h1>Ship price: {ship}$</h1>
        <h1>Total price: {discountPrice > 0 ? discountPrice : totalPrice + (ship > 0 ? ship : 0)}$</h1>
        <p>Tax 10$</p>
        <form action="" onSubmit={handleUseCoupon}>
            <input type="text" placeholder='sale coupon' value={input} onChange={handleChange}/>
            <button>add</button>
        </form>
    </div>
  )
